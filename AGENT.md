...
- **Host-aware root with portal fallback** — `app/page.tsx` is a client component that server-renders `PortalHome` as the static fallback, then reads `window.location.hostname` on mount to switch `/` to the tools or dot landing when served from `tools.iid.sh` or `dot.iid.sh`. It also updates `document.title` and the description meta tag client-side for the active host.
...
- **Props over context for link variants** — `ToolCard` accepts `rootPath` prop, `ToolShell` accepts `backHref` prop, avoiding global context for link targets.
